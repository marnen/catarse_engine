...
describe Catarse::UpdatesController do
  ...
  describe "POST create" do
    ...
    context 'When user is a guest' do
      it{ Catarse::Update.where(:project_id => update.project.id).count.should == 1}
    end
    ...
    context 'When user is admin' do
      let(:current_user) { FactoryGirl.create(:user, admin: true) }
      it{ Catarse::Update.where(:project_id => update.project.id).count.should ==  2}
    end
