...
describe Catarse::Projects::BackersController do
  render_views
  ...
  describe "PUT credits_checkout" do
    ...
    context "with correct user but insufficient credits" do
      ...
      it('should set flash failure'){ request.flash[:failure].should == I18n.t('catarse.projects.backers.checkout.no_credits') }
    ...
    context "with correct user and sufficient credits" do
      ...
      it('should set flash success'){ request.flash[:success].should == I18n.t('catarse.projects.backers.checkout.success') }
  ...
  describe "POST create" do
    ...
    context "when user is logged in" do
      let(:user){ FactoryGirl.create(:user) }
      its(:body){ should =~ /#{I18n.t('catarse.projects.backers.create.title')}/ }
  ...
  describe "GET new" do
    ...
    context "when project.online? is true and we have configured a secure create url" do
      ...
      it "should assign the https url to @create_url" do
        assigns(:create_url).should == project_backers_url(project, host: Catarse::Configuration[:secure_review_host], protocol: 'https')
      end
    end
