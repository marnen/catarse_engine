...
describe Catarse::ProjectObserver do
  ...
  describe "after_create" do
    ...
    let(:user) { FactoryGirl.create(:user, email: Catarse::Configuration[:email_projects])}
  ...
  describe '#notify_owner_that_project_is_successful' do
    ...
    before do
      Catarse::Configuration[:facebook_url] = 'http://facebook.com/foo'
      Catarse::Configuration[:blog_url] = 'http://blog.com/foo'
    ...
    it "should create notification for project owner" do
      Catarse::Notification.where(user_id: project.user.id, notification_type_id: project_success.id, project_id: project.id).first.should_not be_nil
    end
  end
