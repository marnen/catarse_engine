...
task :send_old_posts_to_tumblr => :environment do
  ...
  Comment.posts.each do |post|
    Tumblr::Post.create(TumblrUser, group: Catarse::Configuration[:tumblr_blog], title: post.title, body: post.comment, date: post.created_at, tags: post.project.to_param)
  end
