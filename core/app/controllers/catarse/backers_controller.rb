...
class BackersController < BaseController
  ...
  def index
    index! do |format|
      format.json{ return render :json => @backs.includes(:user, :reward, project: [:user, :category, :project_total]).to_json({:include_project => true, :can_manage => (current_user == @user)}) }
