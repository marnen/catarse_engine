...
class ProjectsController < BaseController
  ...
  load_and_authorize_resource :project, only: [ :new, :update, :destroy ]
  ...
  def last_tweets
    Rails.cache.fetch('last_tweets', :expires_in => 30.minutes) do
      begin
        JSON.parse(Net::HTTP.get(URI("http://api.twitter.com/1/statuses/user_timeline.json?screen_name=#{Catarse::Configuration[:twitter_username]}")))[0..1]
      rescue
        []
      end
    end
